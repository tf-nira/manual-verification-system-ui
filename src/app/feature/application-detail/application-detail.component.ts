...
export class ApplicationDetailComponent implements OnInit {
  ...
  principalDependentField =PRINCIPAL_DEPENDENT_FIELDS;
  schoolDetailField =SCHOOL_DETAIL_FIELDS;
  configData:any={};
...
additionalFetchedDocuments: { category: string; title: string; fileName: string; file: SafeResourceUrl | null }[] = [];
  ...
  constants = {
    ...
    Modify_DETAILS,
    SERVICE_CONST_DEACTIVATED,
    SERVICE_TYPE_ALIEN_DEACTIVATED,
    SERVICE_TYPE_ALIEN_REPLACEMENT
  }
...
getTitlesForDocument(document: any): string[] {
 ...
 saveModifiedRowData(changes: any,rowData: any){
  ...
  handleRecordsClick(): void {
    ...
    // Deactivated / Alien Deactivated: use AIN as the registration ID
    if (this.service === SERVICE_CONST_DEACTIVATED && this.serviceType === SERVICE_TYPE_ALIEN_DEACTIVATED) {
      const ain = this.rowData?.demographics?.AIN;
      if (ain) {
        const role = 'alien';
        if (!this.cachedDemographicsData || !this.cachedDemographicsData[role]) {
          this.fetchDemographicData(ain, role);
        } else {
...
formatLabel(key: string): string {
  ...
  // Check standard field label map
  if (FIELD_LABEL_MAP[key]) {
    return FIELD_LABEL_MAP[key];
  }
  
  // Fallback: auto-format the key
  return key
    .replace(/([A-Z])/g, ' $1')
